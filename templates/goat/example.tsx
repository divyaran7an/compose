...
import { Bot, Wallet, Send, User, Zap, ArrowRightLeft, DollarSign, Copy, AlertCircle, Coins, Rocket } from 'lucide-react';
...
export default function GoatAgent() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/goat', {
        method: 'POST',
        headers: {
  ...
  const quickActions = [
    { icon: Wallet, text: 'Check my balance', action: 'Check my balance' },
    { icon: Send, text: 'Send 0.01 ETH', action: 'Send 0.01 ETH to 0x...' },
    { icon: ArrowRightLeft, text: 'Swap tokens', action: 'Swap 10 USDC for PEPE' },
    { icon: DollarSign, text: 'Wallet address', action: 'What\'s my wallet address?' }
  ];
      ...
      <div className="relative flex flex-col min-h-screen">
          ...
          <div className="mx-auto max-w-6xl text-center">
            <h1 className="font-heading text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl">
              <span className="bg-gradient-to-b from-white to-zinc-400 bg-clip-text text-transparent">
                Ship Onchain AI Agents
              </span>
            </h1>
            <p className="mt-6 max-w-2xl mx-auto text-lg text-zinc-400 md:text-xl">
              GOAT configured. Wallet connected. Execute blockchain transactions through natural conversation.
            </p>
        ...
        <div className="flex-1 px-6 pb-6">
          <div className="mx-auto max-w-4xl h-full flex flex-col gap-4">
            ...
            {/* Chat Container */}
            <Card className="flex-1 border-zinc-800 bg-zinc-900/50 backdrop-blur flex flex-col min-h-0">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-zinc-400">
                      AI agent that executes real blockchain transactions • Send tokens, swap assets, check balances
                    </CardDescription>
                  </div>
              ...
              <CardContent className="flex-1 p-4 overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-4">
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center p-8">
                      ...
                      <h3 className="text-lg font-medium text-zinc-200 mb-2">Ready to transact</h3>
                      <p className="text-sm text-zinc-400 max-w-sm mb-6">
                        I can execute blockchain transactions through natural conversation. Try the quick actions below!
                      </p>
                      <div className="grid grid-cols-2 gap-2 w-full max-w-md">
                        {quickActions.map((action, index) => (
                          <button
                            ...
                            className="flex items-center gap-2 p-3 text-sm bg-zinc-800/50 hover:bg-zinc-800 rounded-lg text-zinc-300 hover:text-white transition-colors text-left"
                          >
                    ...
                    <>
                      {messages.map((message) => (
                        ...
                        >
                          ...
                          <div className={`group flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'} max-w-[85%] md:max-w-[80%]`}>
                            <div
                            ...
                            <span className="text-xs text-zinc-500 mt-1 px-2">
                              {message.role === 'user' ? 'You' : 'Agent'} • {formatTime(message.createdAt)}
                            </span>
                          </div>
                          {message.role === 'user' && (
                            <div className="h-8 w-8 rounded-full bg-zinc-800 flex items-center justify-center flex-shrink-0">
                      ...
                      {isLoading && (
                        <div className="flex items-start gap-3">
                          ...
                          <div className="bg-zinc-800 text-zinc-100 px-4 py-2 rounded-2xl">
                            <div className="flex items-center gap-2">
                              <span className="text-sm">Processing transaction</span>
                              <div className="flex space-x-1">
                                <div className="h-2 w-2 bg-zinc-500 rounded-full animate-bounce [animation-delay:-0.3s]" />
                                <div className="h-2 w-2 bg-zinc-500 rounded-full animate-bounce [animation-delay:-0.15s]" />
                ...
                <form onSubmit={onSubmit} className="mt-4 flex gap-2">
                  <Input
                    ...
                    placeholder="Ask me to send tokens, check balances, or swap assets..."
                    disabled={isLoading}
                    className="bg-zinc-800 border-zinc-700 text-white placeholder-zinc-400 focus:border-white"
                    onKeyDown={(e) => {
              ...
              <CardContent className="pt-6">
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="text-center">
                    <div className="mx-auto mb-2 h-10 w-10 rounded-full bg-zinc-800 flex items-center justify-center">
                      <DollarSign className="h-5 w-5 text-zinc-400" />
                    </div>
                    <h3 className="font-medium text-sm">Token Operations</h3>
                    <p className="text-xs text-zinc-400 mt-1">
                      Send ETH, check balances, manage tokens
                    </p>
                  </div>
                  <div className="text-center">
                    <div className="mx-auto mb-2 h-10 w-10 rounded-full bg-zinc-800 flex items-center justify-center">
                    ...
                    <h3 className="font-medium text-sm">Token Swaps</h3>
                    <p className="text-xs text-zinc-400 mt-1">
                      Swap USDC for PEPE, trade through DEXs
                    </p>
                  </div>
                  <div className="text-center">
                    <div className="mx-auto mb-2 h-10 w-10 rounded-full bg-zinc-800 flex items-center justify-center">
                    ...
                    <h3 className="font-medium text-sm">Instant Execution</h3>
                    <p className="text-xs text-zinc-400 mt-1">
                      Natural language to blockchain transactions
                    </p>
                  </div>
                </div>
                <p className="text-center text-xs text-zinc-500 mt-4">
                  🛡️ Running on testnet for safe experimentation
                </p>
              </CardContent>
            </Card>
          </div>
